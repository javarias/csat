...
Freemode::Freemode(Communication *com) : _tid(0)
{
	this->com = com;
}
			...
			switch(event.type){
				...
				case SDL_KEYUP:		//A key has been released
					if(!strcmp("up",SDL_GetKeyName(event.key.keysym.sym))){
						...
						com->write(wref,162,1,1,1,1,NULL);
						else com->write(wref,162,1,0,1,1,NULL);
						}
					if(!strcmp("down",SDL_GetKeyName(event.key.keysym.sym))){
						if(event.key.type == SDL_KEYDOWN) 
						com->write(wref,162,1,1,0,1,NULL);
						else com->write(wref,162,1,0,0,1,NULL);
						}
					if(!strcmp("right",SDL_GetKeyName(event.key.keysym.sym))){
						if(event.key.type == SDL_KEYDOWN) 
						com->write(wref,164,1,1,1,1,NULL);
						else com->write(wref,164,1,0,1,1,NULL);
						}
					if(!strcmp("left",SDL_GetKeyName(event.key.keysym.sym))){
						if(event.key.type == SDL_KEYDOWN) 
						com->write(wref,164,1,1,0,1,NULL);
						else com->write(wref,164,1,0,0,1,NULL);
						}
