...
\item \textbf{Recursos Disponibles:} c\'omo estamos trabajando en un proyecto
  estudiantil, no disponemos de grandes recursos monetarios para
  gastar en gesti\'on formal.
...
\item \textbf{Proyecci\'on en el tiempo:} se cuenta con un plazo determinado,
  debido a que el proyecto pretende ser parte de la Feria de Software
  2006, de no m\'as de cinco meses a partir de la entrega de este
  informe.
\item \textbf{Comprensi\'on de la estructura interior del producto por el cliente:}
  el cliente debe ser capaz de adaptar el programa a futuros cambios en
  el medio ambiente, para permitir manejar a otro tipo de robot, por
  ejemplo, o para manejar un nuevo dispositivo (como un nuevo sensor) en
  un robot ya existente. Por esto, se necesitar\'a documentaci\'on del
  dise\~no del programa tanto como del c\'odigo y su utilizaci\'on.
\end{itemize}

Teniendo en cuenta el conjunto de caracter\'isticas del proyecto, se
decide usar un m\'etodo \'agil de desarrollo \cite{Cockburn02} para la
...
incremental}) \cite{Gilb88,Shore04} en el trabajo. Sin embargo, no es posible
